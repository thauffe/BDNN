...
##############################################
# Rates through time following a logistic trajectory
time = np.arange(0.0, 35.0, step=0.01)

L = 0.4
l = 0.1
k = 0.5
x0 = 20.0 # midpoint
sp_rate = (L - l) / (1 + np.exp(-k * (time - x0))) + l
fixed_Ltt = np.column_stack((time, sp_rate))[::-1, :]

M = 0.4
m = 0.05
k = -0.5
x0 = 15.0 # midpoint
ex_rate = (M - m) / (1 + np.exp(-k * (time - x0))) + m
fixed_Mtt = np.column_stack((time, ex_rate))[::-1, :]
